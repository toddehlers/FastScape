...
do j=j1,j2
  ...
  sup(i)=-factxp
  inf(i)=-factxm
  f(i)=t%h(ij)+factyp*t%h(ijp)-(factyp+factym)*t%h(ij)+factym*t%h(ijm)
  enddo
diag(1)=1.d0
sup(1)=s4
f(1)=t%h((j-1)*p%nx+1)*f4
diag(p%nx)=1.d0
inf(p%nx)=s2
f(p%nx)=t%h((j-1)*p%nx+p%nx)*f2
